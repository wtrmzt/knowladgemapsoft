...
import type { Memo, CustomNodeData, KnowledgeMap, ApiNode } from '../types';
import { Loader2, Zap, Cog, Award, History, FileText, X, Brain, Info as InfoIcon } from 'lucide-react';
import { motion } from "framer-motion"; // アニメーションのためframer-motionを導入（推奨）
...
function DashboardPage() {
  ...
  const [mapDataFromApi, setMapDataFromApi] = useState<ApiKnowledgeMapResponse | null>(null);
  ...
  const [isProcessing, setIsProcessing] = useState(false); // メモ保存・マップ生成中の状態
  ...
  useEffect(() => {
    ...
    const transformedNodes: CustomNodeType[] = apiMapData.nodes.map(node => ({
        id: String(node.id),
    ...
    const transformedEdges: Edge[] = (apiMapData.edges || []).map((edge, i) => ({
  ...
  const handleSaveMemo = useCallback(async (text: string) => {
    if (!text.trim()) {
      toast({ title: "エラー", description: "メモ内容が空です。", variant: "destructive" });
      return;
    }
    // ★★★ ログ記録を追加 ★★★
    loggingService.logActivity('CREATE_MEMO', { memoLength: text.length });
    
    setMapDataFromApi(null);
    try {
      const savedMemo = await memoService.createMemo(text);
      setCurrentMemo(savedMemo);
      toast({ title: "成功", description: "メモを保存しました。" });
    } catch (error: any) {
      toast({ title: "エラー", description: `メモの保存に失敗しました: ${error.message}`, variant: "destructive" });
    }
  }, [toast]);
  ...
  const handleGenerateMap = useCallback(async () => {
    ...
    // ★★★ ログ記録を追加 ★★★
    loggingService.logActivity('GENERATE_MAP', { memoId: currentMemo.id });

    setIsLoadingMap(true);
    setMapDataFromApi(null);
    try {
      const apiResponse = await mapService.generateMap(currentMemo.id);
      setMapDataFromApi(apiResponse); 
      toast({ title: "成功", description: "知識マップを生成しました。" });
    } catch (error: any) {
      toast({ title: "エラー", description: `マップ生成に失敗しました: ${error.message || '不明なエラー'}`, variant: "destructive" });
    } finally {
  ...
  const handleNodeAdded = useCallback((newNode: CustomNodeType, newEdge: Edge) => {
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(() => {
      ...
      mapService.updateMap(currentMemo.id, mapDataToSave)
        ...
        .catch((err) => toast({ title: "自動保存に失敗しました", variant: "destructive" }))
